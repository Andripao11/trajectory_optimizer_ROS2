...
void optimizer::init(){
  ...
  RCLCPP_WARN(rclcpp::get_logger("TROsolver"), "TROsolver ELAPSED TIME: %f s", elapsed.count());
...
void optimizer::parameters(){
    ...
    /* DEBUG TEST ----------------------------------------- */
  const char *manual = std::getenv("TRO_CSV_TEST");   // variabile d’ambiente
  if (manual && std::strlen(manual) > 0) {
    curvaturePath = std::string(manual);              // sovrascrivo il path
    RCLCPP_WARN(rclcpp::get_logger("TROsolver"),
                "DEBUG: uso path CSV forzato: %s", curvaturePath.c_str());
  }
  ...
  softCurvature = sg_smooth(rawCurvature, 19, 5); //19, 5
...
void optimizer::Solve(){
  ...
  for(int e = 0; e < N; e++){
      ...
      //RCLCPP_INFO(rclcpp::get_logger("TROsolver"), "Valore obiettivo finale: %.4f", solver.stats()["f"].to_double());
  }
...
void optimizer::debug(){
  ...
  // Horizon Length:
  //RCLCPP_INFO(this->get_logger(), "Horizon Length: %i", N);
...
MatrixXd optimizer::read_csv(std::string& filename, bool longvec, bool firstout){ 
    ...
    if(!myFile.is_open()) {
    std::cerr << "OPTIMIZER::read_csv – open() failed for "
              << filename << " : " << std::strerror(errno)
              << " (errno=" << errno << ")\n";
    throw std::runtime_error("OPTIMIZER::read_csv Could not open file");
    }
...
void optimizer::save_info(std::string& filePath, std::string name){
  ...
  try{
    ...
    {
        RCLCPP_ERROR(logger_, "OPTIMIZER::save_info – exception: %s", e.what());
    }
}
