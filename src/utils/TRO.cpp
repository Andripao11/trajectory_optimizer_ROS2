...
void TRO::init(){
    ...
    RCLCPP_WARN(rclcpp::get_logger("TRO"), "TRO ELAPSED TIME: %f ms", elapsed.count()*1000);
...
void TRO::get_data(){
    ...
    std::cout << "reading optimizedPath!\n";
...
void TRO::join_track(const slam_msgs::msg::PositionCarLandmark::SharedPtr data){

    // Heading of the spline (polynomial) at the start (bs = [bxs,bys])
    pair<double,double> bs = coef_bx_by(data->yaw_car);
    ...
    if(firstSpId > 0){
        ...
        traj.coefsFirstX = coefs_first(data->x_car, polyval(traj.coefsSplinesTrajX.row(firstSpId-1),1.0), bs.first, bf.first);
        traj.coefsFirstY = coefs_first(data->y_car, polyval(traj.coefsSplinesTrajY.row(firstSpId-1),1.0), bs.second, bf.second);
    ...
    while(std::getline(myFile, line)){
        ...
        while(std::getline(ss,val,',')){
            try {
                ...
                std::cerr << "Errore conversione std::stod per valore: '" << val << "' nella riga: " << line << std::endl;
                throw; // rilancia l’eccezione dopo il messaggio
            }
        }
...
void TRO::save_csv(string filepath){
    try{
        ...
        RCLCPP_INFO(rclcpp::get_logger("trajectory_optimizer"), "OPTIMAL TRACK data SAVED AT: %s", filepath.c_str());
    }
    catch (const std::exception& e)
    {
        RCLCPP_ERROR_STREAM(rclcpp::get_logger("trajectory_optimizer"), "TRO::save_csv Exception was thrown: \n" << e.what());
    }
}
...
// Publish optimized path and cones for visualization
void TRO::publish_visualization(rclcpp::Node::SharedPtr node) {
    if (!isRun) return;

    // --- Traiettoria ottimizzata ---
    nav_msgs::msg::Path pathMsg;
    pathMsg.header.stamp = node->get_clock()->now();
    ...
    // --- MarkerArray per i coni ---
    visualization_msgs::msg::MarkerArray ma;
    int id = 0;
    for (int i = 0; i < traj.pointsTraj.rows(); i++) {
        double x = traj.pointsTraj(i, 0);
        double y = traj.pointsTraj(i, 1);
        double heading = traj.Pheading(std::min(i, (int)traj.Pheading.size() - 1));
        double fl = traj.freeL(i);
        double fr = traj.freeR(i);

        // LEFT CONE (blue)
        visualization_msgs::msg::Marker m_left;
        m_left.header.frame_id = "map";
        m_left.header.stamp = node->get_clock()->now();
        m_left.id = id++;
        m_left.type = visualization_msgs::msg::Marker::CYLINDER;
        m_left.action = visualization_msgs::msg::Marker::ADD;
        m_left.pose.position.x = x - fl * sin(heading);
        m_left.pose.position.y = y + fl * cos(heading);
        ...
        // RIGHT CONE (yellow)
        visualization_msgs::msg::Marker m_right = m_left;
        m_right.id = id++;
        m_right.pose.position.x = x + fr * sin(heading);
        m_right.pose.position.y = y - fr * cos(heading);
        m_right.color.r = 1.0;
        m_right.color.g = 1.0;
        m_right.color.b = 0.0;
...
path_interfaces::msg::ObjectiveArrayCurv TRO::plannerTRO(const slam_msgs::msg::PositionCarLandmark::SharedPtr data){

    // Create message
    path_interfaces::msg::ObjectiveArrayCurv msg;
    ...
    for(int i=0; i < steps; i++){
        path_interfaces::msg::ObjectiveCurv objective;
...
MatrixXd TRO::planning(const slam_msgs::msg::PositionCarLandmark::SharedPtr data){
...
void TRO::pubMarkerArray(const slam_msgs::msgs::PositionCarLandmark::SharedPtr &conesLoop, rclcpp::Publisher<visualization_msgs::msg::MarkerArray>::SharedPtr pubConesLoopMA) {
    ...
    for (int i = 0; i < conesLoop.left.size(); i++) {
        m.id = id++;
        m.pose.position.x = conesLoop.blue_x_coord[i];
        m.pose.position.y = conesLoop.blue_y_coord[i];
    ...
    for (int i = 0; i < conesLoop.right.size(); i++) {
        m.id = id++;
        m.pose.position.x = conesLoop.conesLoop.yellow_x_coord[i];
        m.pose.position.y = conesLoop.conesLoop.yellow_x_coord[i];
        ma.markers.push_back(m);
    }
