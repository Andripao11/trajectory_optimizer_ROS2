...
#define ROS_WARN(fmt, ...)  printf(fmt "\n", ##__VA_ARGS__)
...
void GRO::init(as_msgs::Tracklimits &data){
    ...
    ROS_INFO_STREAM("Start get_data()");
    get_data(data);
    ROS_INFO_STREAM("Done get_data(), pointsTraj rows: " << traj.pointsTraj.rows());

    ROS_INFO_STREAM("Start get_trajectory()");
    get_trajectory();
    ROS_INFO_STREAM("Done get_trajectory(), pointsTraj rows: " << traj.pointsTraj.rows());

    ROS_INFO_STREAM("Start radi_curv()");
    radi_curv();
    ROS_INFO_STREAM("Done radi_curv(), radiCurv size: " << traj.radiCurv.size());

    ROS_INFO_STREAM("Start velocity_profile()");
    velocity_profile();
    ROS_INFO_STREAM("Done velocity_profile(), freeR size: " << traj.freeR.size());
...
void GRO::get_data(as_msgs::Tracklimits &data) {
   ...
   std::cout << "[DEBUG] Inizio get_data()" << std::endl;

    //std::cout << "[DEBUG] Numero coni: " << data.x_left.size() << std::endl;

    std::cout << "[DEBUG] Prima di gate_generator()" << std::endl;
    gate_generator(data);
    std::cout << "[DEBUG] Dopo gate_generator()" << std::endl;



    if (solverFlag){
        std::cout << "[DEBUG] Applico carWidth e securityFactor" << std::endl;
    ...
    std::cout << "[DEBUG] Fine get_data()" << std::endl;
}

// Principal function: apply algorithm of minimum curvature
void GRO::get_trajectory(){

    std::cout << "[DEBUG] get_trajectory() called" << std::endl;
...
void GRO::gate_generator(as_msgs::Tracklimits &data){
    ...
    std::cout << "[DEBUG] FLAG 1 gate_generator" << std::endl;

    MatrixXd conesRight(data.right.size(),2), conesLeft(data.left.size(),2);

    std::cout << "[DEBUG] FLAG 2 gate_generator" << std::endl;
    ...
    std::cout << "[DEBUG] FLAG 3 gate_generator" << std::endl;
    ...
    std::cout << "[DEBUG] FLAG 4 gate_generator" << std::endl;
    ...
    std::cout << "[DEBUG] FLAG 5 gate_generator" << std::endl;
    ...
    std::cout << "[DEBUG] FLAG 6 Inizio interpolazione" << std::endl;


    // Interpolate track limit reference with splines
    MatrixXd interpolatedRefCoefsX = coefs_splines(conesRef.col(0));
    std::cout << "[DEBUG] FLAG 6.1: interpolatedRefCoefsX OK" << std::endl;

    MatrixXd interpolatedRefCoefsY = coefs_splines(conesRef.col(1));
    std::cout << "[DEBUG] FLAG 6.2: interpolatedRefCoefsY OK" << std::endl;

    MatrixXd interpolatedOthCoefsX = coefs_splines(conesOther.col(0));
    std::cout << "[DEBUG] FLAG 6.3: interpolatedOthCoefsX OK" << std::endl;

    MatrixXd interpolatedOthCoefsY = coefs_splines(conesOther.col(1));
    std::cout << "[DEBUG] FLAG 6.4: interpolatedOthCoefsY OK" << std::endl;

    std::cout << "[DEBUG] FLAG 7 gate_generator" << std::endl;
    ...
    std::cout << "[DEBUG] FLAG 8 gate_generator" << std::endl;
    ...
    std::cout << "[DEBUG] FLAG 9 gate_generator" << std::endl;
...
std::cout << "[DEBUG] Arrivato alla fine" << std::endl;
}
...
MatrixXd GRO::coefs_splines(VectorXd x){
    ...
    // Getting the lenght
        int N = x.size();
        std::cout << "[DEBUG][coefs_splines] N = " << N << std::endl;
    ...
    // Building Deqs matrix
        ...
        for(int i=2; i<=N-1; i++) {
            ...
            if (i % 100 == 0) {
                std::cout << "[DEBUG] Costruzione Deqs: step " << i << " / " << N << std::endl;
            }
        }
        // Building corners:
        Deqs.topLeftCorner(4,5) = matrixEqs.rightCols(5);
    ...
    // Obtaining coefficients
        ...
        // coefsVec = Dinv*xp;
        std::cout << "[DEBUG] Inizio risoluzione sistema lineare (" << 4*N << "x" << 4*N << ")" << std::endl;
        coefsVec = Deqs.partialPivLu().solve(xp);
        std::cout << "[DEBUG] Sistema risolto" << std::endl;
    ...
    // Reshape and reorganizing
        ...
        std::cout << "[DEBUG][coefs_splines] Done in " << duration << " seconds\n";
...
as_msgs::Tracklimits GRO::read_csv(const std::string &filename){
    std::cout << "Trying to open file: " << filename << std::endl;
    std::ifstream file(filename);
    if (!file.is_open()) {
        throw std::runtime_error("GRO::read_csv Could not open file");
    }
    ...
    while (std::getline(file, line)) {
        ...
        as_msgs::Cone cone;
        cone.position_global.x = x;
        cone.position_global.y = y;
    ...
    if (!midlineMsg.left.empty() &&
        midlineMsg.left[0].position_global.x == midlineMsg.left[midlineMsg.left.size() - 1].position_global.x &&
        midlineMsg.left[0].position_global.y == midlineMsg.left[midlineMsg.left.size() - 1].position_global.y)
        midlineMsg.left.pop_back();

    if (!midlineMsg.right.empty() &&
        midlineMsg.right[0].position_global.x == midlineMsg.right[midlineMsg.right.size() - 1].position_global.x &&
        midlineMsg.right[0].position_global.y == midlineMsg.right[midlineMsg.right.size() - 1].position_global.y)
        midlineMsg.right.pop_back();
